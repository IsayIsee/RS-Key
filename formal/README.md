...
| `crates/rsk-piv/src/lib.rs:368` | `deselect` → `()`: the verified PIN outlives its selection | the same |
...
| Mutation switch | Removes | Target invariant | Caught in |
|---|---|---|---|
| `BugUseWhenBlocked` | the `left == 0 => PIN_BLOCKED` floor (`crates/rsk-piv/src/lib.rs:520-522` / `crates/rsk-openpgp/src/pin.rs:232-234`), which guards a direct verify AND a recovery reference | `NoAuthWhenBlocked` | 30 states |
