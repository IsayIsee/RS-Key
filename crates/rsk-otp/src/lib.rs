...
pub fn slot_status<S: Storage>(dev: &Device, fs: &mut Fs<S>) -> [SlotStatus; 4] {
    ...
    for (i, fid) in (EF_OTP_SLOT1..=EF_OTP_SLOT_LAST).enumerate() {
        // A medium that could not answer reads as Empty on this read-only
        // snapshot: it has no error channel, and a host's own readers are what
        // report a faulted store.
        let Ok(Some(_)) = try_read_slot(dev, fs, fid, &mut buf) else {
            continue;
        };
        let tkt = buf[OFF_TKT_FLAGS];
        let cfg = buf[OFF_CFG_FLAGS];
